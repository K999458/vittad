...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument('--dataset_file', default='tad')
...
def main(args):
    ...
    if not args.eval and not args.test:
        ...
        # 使用训练集的COCO API作为base_ds
        base_ds = dataset_train.coco if hasattr(dataset_train, 'coco') else None
        if base_ds is not None:
            print("成功获取训练集COCO API")
        else:
            print("警告：无法获取训练集COCO API")
    else:
        dataset_train = None
        data_loader_train = None
        dataset_val = build_dataset(image_set='val', args=args)
        base_ds = dataset_val.coco if hasattr(dataset_val, 'coco') else None
    ...
    if args.dataset_file == "coco_panoptic":
        ...
        # 直接从数据集获取COCO对象
        if dataset_val is not None and hasattr(dataset_val, 'coco'):
            base_ds = dataset_val.coco
            print("成功从验证集获取COCO API")
        else:
            print("无法获取验证集COCO API，将跳过评估")
            base_ds = None
