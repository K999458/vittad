...
# -*- coding: utf-8 -*-
"""
build_jig_docx.py —— 《中国图象图形学报》(JIG) 投稿初稿构建脚本（JIG_FORK_MARK）

由 build_oep_docx.py v12 fork。机械性改写见 _fork_transform.py；语义性改写：
  著者-出版年制引用与字母序参考文献、结构化中英文摘要（目的/方法/结果/结论，
  英文摘要约 1000 词）、JIG 头区（中图法分类号行/论文引用格式行/单位 6 号）、
  章节自 0 引言重编号、图表题注体例（Table 全拼、表题加粗宋体、图题宋体）、
  尾区（参考文献(References) 字母序 8pt、作者简介、致谢占位）。
  （jig-v2：大空白与丢字清除——v1 渲染五处超 3 cm 空白带（第 4 页右栏
  5.4、第 5 页左栏 15.8、第 10 页整幅 5.6、第 12 页右栏 6.0、第 13 页右栏
  10.6 cm），根因是 JIG 头区较 OEP 版多约一页使图 1 显示页后移、锚位滞留
  前页触发"浮动屏障"连锁；图 1 流位推迟至 1.2 节第 2 段后，五处空白连锁
  消除（16 页全部 <3 cm）。另修复 2.5 节"标出真值缺陷区域"句内多余空格
  触发的引擎丢字（"值缺"二字）。新增 _qa/text_integrity_check.py 逐段
  文字完整性校验（零宽幽灵滤除+页眉页脚剔除+三级判定），v2 全文 1 044 段
  0 丢字；whitespace_scan 16 页 0 超标。正文文字与图表数据零变化。）
  （jig-v3：公式 OMML 化 + 体例勘误——移植 OEP v13/v14 批次中适用于本刊的
  全部修复。①公式表示：式 (1)–(11) 由 Unicode 文本占位（上下标靠字符格式、
  分式写成斜线、Σ 上下限挤在行内）全部改为 Word 原生公式对象（OMML，即
  Word"插入→公式"工具的格式）重排——真分式、求和号上下限置于其上下
  （undOvr）、结构化上下标与波浪饰符，函数名 exp/min/mean/std/median 直立、
  变量斜体（公式引擎自动式样，落实 JIG 模板要求 1"变量用斜体"与要求 2
  "公式建议用公式编辑器编辑"）；公式居中制表位取 JIG 栏宽 8.13 cm 的真中点
  4.07 cm，编号仍右贴栏缘；式 (9) 集合式较宽居中位左移 0.37 cm；句末标点
  （，/。）与式 (7)/(8) 的"时，"以正文宋体夹排于数学区之外。②参考文献英文
  题名由原文 Title Case 统一转 JIG 2021 著录样例的句首大写式（sentence
  case；缩写/驼峰/含数字词与专名保留），"IEEE TPAMI"缩写经 JOURNAL_MAP
  展开为全称。③"命题 1."半角句点改"命题 1"后接全角空格（定理体例）。
  ④ε₀ 的 Unicode 下标字符（U+2080）改格式化下标 ε_{0}（两处），表格单元格
  fitness² 的 Unicode 上标 ²（U+00B2）统一为格式化上标 fitness^{2}（两处）。
  ⑤作者行通讯标记并入上标（^{1}* → ^{1*}，中英两处）。⑥指标英文全称
  Title Case 转小写式（Area Under the ROC Curve → area under the ROC
  curve、Average Precision → average precision，专名/缩写除外）。⑦"宏观
  点级均值"改"点级宏均值"（与全文"宏均值"术语一致）。⑧三线表表头行设
  tblHeader，跨页/跨栏按行续排时自动重复表头。⑨OMML 化后行高增加、分页
  下移的浮动锚位联动：构建后经 LibreOffice 渲染逐页复扫空白带与丢字
  （_qa/whitespace_scan.py + text_integrity_check.py）确认后交付。
  正文其余文字与图表数据零变化。v1/v2 保留）
...
TEMPLATE = os.path.join(BASE, 'template_extract', 'jig_tpl_2024.docx')
DRAFT = os.path.join(ROOT, 'DRAFT_v1_zh.md')
BIB = os.path.join(ROOT, 'references.bib')
FIGDIR = os.path.join(ROOT, 'figures')
OUT = os.path.join(BASE, 'GCAS_中国图象图形学报_排版初稿_v3_20260831.docx')

# 页面几何（由模板 sectPr 实测）：A4，正文宽 9694 twip = 17.13 cm；
# 双栏栏宽 (9694-236)/2 = 4729 twip = 8.34 cm
FULL_W = Cm(16.6)      # 通栏图/表宽度（JIG 版心 17.0 cm，留余量）
COL_W = Cm(7.9)        # 栏宽图宽度（JIG 栏宽 8.13 cm）
COL_W_TW = 4607        # 栏宽（twip）：(9638-424)/2，用于栏内表格
...
def _split_authors(author_field):
    """bib author 字段 -> [(姓, 名首字母串), ...]"""
    out = []
    for a in author_field.split(' and '):
        a = latex_clean(a.strip())
        if ',' in a:
        ...
        out.append((last, ' '.join(initials)))
    return out


def jig_authors(author_field):
    """'Liu, J. and Xie, G. ...' -> 'Liu J, Xie G, Chen R and Li X'。
    JIG 体例（模板要求）：姓前名后、需列出全部作者、英文文献最后一个作者
    前用 and 连接，其余作者用逗号隔开。"""
    names = [(last + ' ' + ini).strip() for last, ini in
             _split_authors(author_field)]
    if len(names) == 1:
        return names[0]
    return ', '.join(names[:-1]) + ' and ' + names[-1]


# jig-v3：英文题名统一转句首大写式（sentence case），对齐 JIG《文后参考
# 文献编排格式》2021 全部著录样例（"Research on several key problems in
# face recognition" 等均为句首大写式）。缩写（AD/SDF/ICP）、驼峰
# （PointNet/ShapeNet）、含数字词（3D/R3D）自动保留原大小写；无法由规则
# 判定的专名列入 TITLE_PROTECT 兜底。冒号/破折号后的子题名首词一律小写。
TITLE_PROTECT = {'Eyecandies'}


def sentence_case_title(ti):
    def protected(word):
        for part in re.split(r'[-–]', word):
            if (any(ch.isdigit() for ch in part)
                    or sum(1 for ch in part if ch.isupper()) >= 2
                    or part in TITLE_PROTECT):
                return True
        return False
...
# 期刊名缩写展开（jig-v3：刊名全称/标准缩写体例内须统一；本 bib 仅 TPAMI
# 一处用缩写，展开为全称与其余条目一致。bib 为只读输入，不改动）
JOURNAL_MAP = {
    'IEEE TPAMI': 'IEEE Transactions on Pattern Analysis and '
...
# 电子资源引用日期（JIG 电子文献著录：[引用日期]. 网址）
CITE_DATE = '2026-08-31'
...
def jig_format(key, etype, f):
    """转《中国图象图形学报》著者-出版年制条目；返回 (text, missing_fields)。

    体例（《文后参考文献编排格式》2021 + 模板要求 [65]）：
      期刊  全体作者. 出版年. 题名. 刊名, 卷(期): 起止页码 [DOI: …]
      会议  全体作者. 出版年. 题名//会议录全称. 出版地: 出版者: 起止页码
      专著  全体作者. 出版年. 书名. 版次 ed. 出版地: 出版单位
      电子  全体作者. 出版年. 题名[EB/OL]. [引用日期]. 获取路径
    英文题名统一转句首大写式（jig-v3，对齐 JIG 2021 著录样例）。"""
    missing = []
    au = jig_authors(f.get('author', ''))
    ti = sentence_case_title(latex_clean(f.get('title', '')))
    year = f.get('year', '')
    pages = latex_clean(f.get('pages', '')) if 'pages' in f else ''
    pages = pages.replace('-', '-').replace('—', '-')
    if etype == 'article':
        jn = latex_clean(f.get('journal', ''))
        jn = JOURNAL_MAP.get(jn, jn)
        vol = f.get('volume', '')
        num = f.get('number', '')
        s = f'{au}. {year}. {ti}. {jn}'
        if vol:
...
CITE_KEYS = []       # 正文实际引用过的键（出现序），供终检与文献表取用
CITE_LABELS = {}     # key -> (名称部, 年份标注)，如 ('Liu 等', '2023')


def init_cite_labels(entries):
    """按（第一作者姓，年）生成正文标注。

    模板体例：1 人 (Taubin，1995)；2 人 (Horwitz 和 Hoshen，2023)；
    3 人及以上 (Liu 等，2023)。同名同年冲突按其字母序文献条目顺序加
    a/b 后缀（本 bib 实际冲突：ISMP/DUSNet 均 Liang 等 2025，
    PointNet/PointNetPP 均 Qi 等 2017）。"""
    base = {}
    for key, (etype, f) in entries.items():
        sn = [last for last, _ in _split_authors(f.get('author', ''))]
        yr = f.get('year', '?')
        if len(sn) == 1:
            name = sn[0]
        elif len(sn) == 2:
            ...
            name = f'{sn[0]} 等'
        base[key] = (name, yr)
    order = {k: jig_format(k, *entries[k])[0].lower() for k in entries}
    groups = {}
    for k, nv in base.items():
        groups.setdefault(nv, []).append(k)
    CITE_LABELS.clear()
    for nv, ks in groups.items():
        if len(ks) == 1:
            CITE_LABELS[ks[0]] = nv
        else:
            for i, k in enumerate(sorted(ks, key=lambda x: order[x])):
                CITE_LABELS[k] = (nv[0], nv[1] + chr(ord('a') + i))


def cite_text(keys):
    """⟦k1,k2⟧ -> '（Liu 等，2023；Qi 等，2017a，2017b）'。
    相邻同名作者合并年份，不同作者以分号分隔（模板样例体例）。"""
    labs = []
    for k in keys:
        k = k.strip()
        if k not in CITE_LABELS:
            raise KeyError(f'引用键 {k} 不在 references.bib 中')
        if k not in CITE_KEYS:
            CITE_KEYS.append(k)
        labs.append(CITE_LABELS[k])
    merged = []
    for name, yr in labs:
        if merged and merged[-1][0] == name:
            merged[-1][1].append(yr)
        else:
            merged.append([name, [yr]])
    return ('（' + '；'.join(f'{n}，' + '，'.join(ys) for n, ys in merged)
            + '）')
...
def emit_runs(par, text, size=10.5, ea=SONG, ascii_=TNR, bold=None,
    ...
    for tok in TOKEN_RE.split(text):
        ...
        if tok.startswith('**') and tok.endswith('**'):
            ...
            # 著者-出版年制：行内正常字号排（Liu 等，2023），非上标
            r = par.add_run(cite_text(tok[1:-1].split(',')))
            set_font(r, TNR, SONG, size, bold=bold, italic=italic)
        elif tok.startswith('^{'):
...
def pset(par, align=None, fli_chars=None, before=None, after=None,
         line=None, snap=None, ind_left=None, hanging=None):
...
def heading1(doc, text):
    """JIG 一级标题：四号（14 pt）宋体加粗（模板："1 一级标题(四号加粗)"，
    样例 run 为宋体加粗）。"""
    p = doc.add_paragraph()
    pset(p, before=13, after=6.5, snap=False, line=300)
    emit_runs(p, text, size=14, ea=SONG, ascii_=TNR, bold=True)
    return p


def heading2(doc, text):
    """JIG 二级标题：5 号（10.5 pt）宋体加粗。"""
    p = doc.add_paragraph()
    pset(p, before=6.5, after=3, snap=False, line=280)
    emit_runs(p, text, size=10.5, ea=SONG, ascii_=TNR, bold=True)
    return p


def heading3(doc, text):
    """JIG 三级标题：5 号（10.5 pt）宋体。"""
    p = doc.add_paragraph()
    pset(p, before=3, after=0, snap=False, line=280)
    emit_runs(p, text, size=10.5, ea=SONG, ascii_=TNR)
    return p


# ------------------------------------------- OMML 公式（Word 原生公式对象）
# jig-v3：式 (1)–(11) 由 Unicode 文本占位改为 Word"插入→公式"工具的原生
# OMML 对象（移植自 build_oep_docx.py v13/v14）：真分式、求和号上下限
# （undOvr）、结构化上下标与波浪饰符；变量由公式引擎按规则自动斜体
# （落实 JIG 模板要求 1"变量用斜体"），函数名（exp/min/mean/std/median）
# 与文字性记号（top-k、obj）直立。句末标点（，/。）与式 (7)/(8) 的
# "时，"不属于数学内容，以正文宋体 run 夹排在数学区之外。

MATH_XMLNS = (
    'xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math" '
    'xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"')
_M_RPR = ('<w:rPr><w:rFonts w:ascii="Cambria Math" w:hAnsi="Cambria Math"/>'
          '<w:sz w:val="21"/></w:rPr>')          # 公式区 10.5 pt
_T_RPR = ('<w:rPr><w:rFonts w:ascii="Times New Roman" '
          'w:hAnsi="Times New Roman"/><w:sz w:val="21"/></w:rPr>')
...
def mv(t):
    """数学 run：变量斜体/数字与运算符直立等式样由公式引擎自动排布。"""
    return f'<m:r>{_M_RPR}<m:t xml:space="preserve">{_mesc(t)}</m:t></m:r>'


def mo(t):
    """直立体数学 run（函数名 exp/min/mean/std/median）。
    用 m:nor（normal text）而非 m:sty="p"——Word 两者均直立，
    LibreOffice 类引擎忽略 sty="p"（仍斜体）但正确渲染 m:nor。"""
    return (f'<m:r><m:rPr><m:nor/></m:rPr>{_M_RPR}'
            f'<m:t xml:space="preserve">{_mesc(t)}</m:t></m:r>')


def mtx(t):
    """公式内普通文本 run（m:nor，不套数学字距/斜体，Times 直立）。"""
    return (f'<m:r><m:rPr><m:nor/></m:rPr>{_T_RPR}'
            f'<m:t xml:space="preserve">{_mesc(t)}</m:t></m:r>')
...
def msbp(e, sub, sup):
    return (f'<m:sSubSup><m:e>{e}</m:e><m:sub>{sub}</m:sub>'
...
def msum(sub, body):
    """求和号，下限置于其下（undOvr），无上限。"""
    return ('<m:nary><m:naryPr><m:chr m:val="∑"/><m:limLoc m:val="undOvr"/>'
            f'<m:supHide m:val="1"/></m:naryPr><m:sub>{sub}</m:sub>'
            f'<m:sup/><m:e>{body}</m:e></m:nary>')
...
def mmin(lim):
    """min 下带极限脚标。"""
    return f'<m:limLow><m:e>{mo("min")}</m:e><m:lim>{lim}</m:lim></m:limLow>'
...
def omath(*chunks):
    return f'<m:oMath {MATH_XMLNS}>' + ''.join(chunks) + '</m:oMath>'


def _tc(base):
    """X_t^(c) 形通道上标记号。"""
    return msbp(mv(base), mv('t'), mv('(c)'))


def _qtx():
    """q_t(x)。"""
    return msb(mv('q'), mv('t')) + mpar(mv('x'))


def _negexp(idx):
    """exp(−βz_idx(x))。"""
    return mfun(mo('exp'),
                mpar(mv('−β') + msb(mv('z'), mv(idx)) + mpar(mv('x'))))


def _gauss():
    """exp(−d(x,y)²/2σ_S²)，指数为真分式。"""
    return mfun(mo('exp'), mpar(mv('−') + mfr(
        msp(mv('d') + mpar(mv('x,y')), mv('2')),
        mv('2') + msbp(mv('σ'), mv('S'), mv('2')))))


_NX = mv('y∈N') + mpar(mv('x'))     # 求和下标 y∈N(x)
_STILDE = mtld(mv('s'))             # s̃

# 每式一个段序列：('m', OMML) 为数学区，('t', 文本) 为正文宋体夹排文字。
# 线性参照（与 v2 文本占位一致）：
#  (1) μ_t^(c)(p) = mean A_t^(c)(p)，
#  (2) σ_t^(c)(p) = std A_t^(c)(p)，
#  (3) z_t^(c)(x) = (r_t^(c)(x) − μ_t^(c)(q_t(x))) / (σ_t^(c)(q_t(x)) + ε)，
#  (4) z_t(x) = Σ_c λ_c·z_t^(c)(x)，
#  (5) w_t(x) = exp(−βz_t(x)) / Σ_j exp(−βz_j(x))，
#  (6) s(x) = Σ_t w_t(x)z_t(x)，
#  (7) β → 0 时，s(x) → (1/M)Σ_t z_t(x)，
#  (8) β → ∞ 时，s(x) → min_t z_t(x)，
#  (9) σ_S = α·median{d(x,y) : x∈S, y∈N(x)∖{x}}，
# (10) s̃(x) = Σ_{y∈N(x)} exp(−d(x,y)²/2σ_S²)·s(y)
#              / Σ_{y∈N(x)} exp(−d(x,y)²/2σ_S²)。
# (11) S_obj = (mean top-k s̃ − mean s̃) / std s̃，
OMML_EQS = {
    1: [('m', omath(_tc('μ'), mpar(mv('p')), mv('='),
                    mfun(mo('mean'), _tc('A') + mpar(mv('p'))))),
        ('t', '，')],
    2: [('m', omath(_tc('σ'), mpar(mv('p')), mv('='),
                    mfun(mo('std'), _tc('A') + mpar(mv('p'))))),
        ('t', '，')],
    3: [('m', omath(_tc('z'), mpar(mv('x')), mv('='),
                    mfr(_tc('r') + mpar(mv('x')) + mv('−')
                        + _tc('μ') + mpar(_qtx()),
                        _tc('σ') + mpar(_qtx()) + mv('+') + mv('ε')))),
        ('t', '，')],
    4: [('m', omath(msb(mv('z'), mv('t')), mpar(mv('x')), mv('='),
                    msum(mv('c'), msb(mv('λ'), mv('c')) + mv('\u22c5')
                         + _tc('z') + mpar(mv('x'))))),
        ('t', '，')],
    5: [('m', omath(msb(mv('w'), mv('t')), mpar(mv('x')), mv('='),
                    mfr(_negexp('t'), msum(mv('j'), _negexp('j'))))),
        ('t', '，')],
    6: [('m', omath(mv('s'), mpar(mv('x')), mv('='),
                    msum(mv('t'), msb(mv('w'), mv('t')) + mpar(mv('x'))
                         + msb(mv('z'), mv('t')) + mpar(mv('x'))))),
        ('t', '，')],
    7: [('m', omath(mv('β'), mv('→'), mv('0'))), ('t', ' 时，'),
        ('m', omath(mv('s'), mpar(mv('x')), mv('→'), mfr(mv('1'), mv('M')),
                    msum(mv('t'), msb(mv('z'), mv('t')) + mpar(mv('x'))))),
        ('t', '，')],
    8: [('m', omath(mv('β'), mv('→'), mv('∞'))), ('t', ' 时，'),
        ('m', omath(mv('s'), mpar(mv('x')), mv('→'),
                    mfun(mmin(mv('t')),
                         msb(mv('z'), mv('t')) + mpar(mv('x'))))),
        ('t', '，')],
    9: [('m', omath(msb(mv('σ'), mv('S')), mv('='), mv('α'), mv('\u22c5'),
                    mfun(mo('median'), mpar(
                        mv('d') + mpar(mv('x,y')) + mv(':') + mv('x∈S,')
                        + mv('y∈N') + mpar(mv('x')) + mv('∖')
                        + mpar(mv('x'), '{', '}'), '{', '}')))),
        ('t', '，')],
    10: [('m', omath(_STILDE, mpar(mv('x')), mv('='),
                     mfr(msum(_NX, _gauss() + mv('\u22c5') + mv('s')
                              + mpar(mv('y'))),
                         msum(_NX, _gauss())))),
         ('t', '。')],
    11: [('m', omath(msb(mv('S'), mtx('obj')), mv('='),
                     mfr(mfun(mo('mean'), mtx('top-') + mv('k')
                              + mtx(' ') + _STILDE)
                         + mv('−') + mfun(mo('mean'), _STILDE),
                         mfun(mo('std'), _STILDE)))),
         ('t', '，')],
}
...
# 式 (9) 集合式较宽：居中位左移 0.37 cm，保证编号在宽字体引擎
# （LibreOffice/WPS 替换字体）下仍与公式同行；其余公式取栏宽真中点
EQ_CENTER = {9: 3.69}


def equation(doc, num):
    """编号公式段：居中制表位（缺省 4.06 cm = JIG 栏宽 8.13 cm 中点）排
    OMML 数学区与夹排文字，编号以右对齐制表位贴双栏栏缘。"""
    EQN_COUNT[0] += 1
    p = doc.add_paragraph()
    pset(p, snap=False, before=3, after=3, line=260)
    ts = p.paragraph_format.tab_stops
    ts.add_tab_stop(Cm(EQ_CENTER.get(num, 4.06)), WD_TAB_ALIGNMENT.CENTER)
    ts.add_tab_stop(Cm(8.13), WD_TAB_ALIGNMENT.RIGHT)   # 编号贴双栏栏缘
    r = p.add_run('\t')
    set_font(r, TNR, SONG, 10.5)
    for kind, payload in OMML_EQS[num]:
        if kind == 'm':
            p._p.append(parse_xml(payload))
        else:
            r = p.add_run(payload)
            set_font(r, TNR, SONG, 10.5)
    r = p.add_run('\t(' + str(num) + ')')
...
def caption_pair(doc, zh, en, size=9):
    """表题注：中英文两行均小五宋体加黑（JIG 模板要求 4：表题中英文对照，
    居中排在表上，小五号宋体加黑）。两行均置 keepNext 与表体绑定。"""
    p1 = doc.add_paragraph()
    pset(p1, align=WD_ALIGN_PARAGRAPH.CENTER, snap=False, before=3, after=0,
         line=240)
    keep_next(p1)
    emit_runs(p1, zh, size=size, ea=SONG, bold=True)
...
def table_note(doc, text, size=9):
    p = doc.add_paragraph()
...
def add_figure(doc, png, width, zh_cap, zh_sub, en_cap, en_sub):
    """图 + 双语题注。题注为简短名词短语（图1 ×××/Fig.1 ×××，OEP 模板样例式），
    分图（a）（b）说明与图例说明按模板要求 5 以中英对照行紧随各自题注给出。"""
    p = doc.add_paragraph()
    pset(p, align=WD_ALIGN_PARAGRAPH.CENTER, snap=False, before=6, after=2,
         line=240)
    run = p.add_run()
    run.add_picture(os.path.join(FIGDIR, png), width=width)
    # JIG 模板要求 5：图题中英对照小五号宋体，分图/图例说明六号
    lines = [(zh_cap, 9)]
    if zh_sub:
        lines.append((zh_sub, 7.5))
    lines.append((en_cap, 9))
    if en_sub:
        lines.append((en_sub, 7.5))
    for i, (text, sz) in enumerate(lines):
        p = doc.add_paragraph()
        pset(p, align=WD_ALIGN_PARAGRAPH.CENTER, snap=False,
             before=3 if i == 0 else 0,
             after=3 if i == len(lines) - 1 else 0, line=240)
        emit_runs(p, text, size=sz, ea=SONG)
...
def add_floating_figure(doc, png, width, zh_cap, zh_sub, en_cap, en_sub,
    ...
    box_tw = str(COL_W_TW) if column else str(FULL_W_TW)  # JIG 栏宽/版心全宽
    # 顶浮块与正文的间距留在其下缘，底浮块留在其上缘
    _float_props(table._tbl, box_tw,
                 horz_anchor='text' if column else 'margin',
                 y_spec='bottom' if column else 'top',
                 top_gap='170' if column else '0',
                 bot_gap='0' if column else '170')
...
def add_floating_figure_band(doc, fig_a, fig_b):
...
def add_floating_table(doc, tag, tables):
    ...
    pset(p1, align=WD_ALIGN_PARAGRAPH.CENTER, snap=False, before=3, after=0,
         line=240)
    emit_runs(p1, zh, size=9, ea=SONG, bold=True)
    p2 = cell.add_paragraph()
    pset(p2, align=WD_ALIGN_PARAGRAPH.CENTER, snap=False, before=0, after=3,
         line=240)
    emit_runs(p2, en, size=9, ea=SONG, bold=True)
    rows = tables[tag]
...
def add_3line_table(doc, rows, total_tw, size, first_col_ratio=None,
    ...
    # jig-v3（移植 OEP v14）：表头行设 tblHeader——内嵌三线表跨页/跨栏
    # 按行续排时自动重复表头，防表头与表体分离
    ...
    for i, row in enumerate(rows):
        for j, cell_text in enumerate(row):
            ...
            # jig-v3（移植 OEP v14）：Unicode 上标 ²（如 fitness²）统一为
            # 格式化上标，与正文 fitness^{2} 一致
            txt = txt.replace('²', '^{2}')
...
# 摘要按 JIG 模板体例分"目的/方法/结果/结论"四要素书写，要素引导词加粗
# （《中英文摘要书写要求》：第三人称、不用"本文/作者"作主语、不出现图表号
# 与参考文献号）。内容与 OEP 版摘要同源重组，数字未改动。
ABSTRACT_ZH_PARTS = [
    ('目的',
     '三维点云异常检测需在仅有少量无缺陷模板的条件下识别并定位缺陷，'
     '基于配准的免训练管线无需训练、可快速适配产线换型，但其评分阶段长期沿用'
     '粗粒度设计，存在正常公差池化过粗、全局配准适配度加权无法抑制局部错配模板、'
     '逐点独立打分忽略缺陷空间连续性三处局限。针对上述问题，提出免训练的几何一致'
     '异常评分（Geometry-Consistent Anomaly Scoring，GCAS）模块，在保持配准与'
     '残差提取环节不变的前提下，以三级算子替换常规评分阶段。'),
    ('方法',
     '第 1 级细粒度公差校准在小邻域（K = 10）内池化跨模板残差统计以估计局部正常'
     '公差，使微小缺陷在标准化后得以保留；第 2 级软最小模板融合对取负标准化残差做'
     '逐点 softmax，在模板间存在分歧处抑制错配模板的伪残差，同时保留跨模板共识性'
     '响应，该算子具有共识保真、插值与单模板污染上界三条可证明性质；第 3 级 kNN '
     '图空间一致性正则化在测试扫描的内在邻居图上以无量纲高斯核做一步扩散，削弱'
     '孤立噪声响应、巩固连续缺陷区域。三个标量参数按基准一次冻结，由仅使用正常'
     '模板的无标签协议选定，从不按类别调参。'),
    ('结果',
     '在 Real3D-AD 基准三随机种子、官方坐标网格协议下，GCAS 将对象级 AUROC 由 '
     '0.893 提升至 0.928，点级 AUROC 由 0.896 提升至 0.958，点级 AP 由 0.479 '
     '提升至 0.580，对象级 AUROC 超过已发表最优结果（0.859）；点级 AUROC 在全部 '
     '12 类上均提升，逐级消融验证了三级算子各自的独立贡献；点级增益可迁移至 '
     'Anomaly-ShapeNet 与 MulSen-AD 两个基准，评分阶段新增耗时不足端到端推理'
     '总耗时的 1%。'),
    ('结论',
     '仅改进评分阶段即可在零训练成本下显著提升基于配准的三维异常检测性能，'
     '适用于少模板、快速换型的高精度工业质检场景。'),
]

KEYWORDS_ZH = '三维异常检测；点云；工业质检；点云配准；异常评分；空间一致性正则化'

# JIG 模板要求英文摘要按 Objective/Method/Result/Conclusion 四要素撰写、
# 约 1000 词（"本刊正在积极申请 EI 等国际数据库收录……英文摘要字数要求为
# 1000 词左右，不需与中文摘要完全对应"）。在 OEP 版约 330 词摘要基础上，
# 按正文第 0–3 章内容扩写；全部数字与正文一致，未引入新事实。
ABSTRACT_EN_PARTS = [
    ('Objective',
     'Three-dimensional anomaly detection on high-resolution point clouds '
     'aims to identify defective objects and localize defect regions, and it '
     'is a key step in quality inspection for high-precision manufacturing. '
     'Because defects are rare, diverse in shape, and expensive to annotate, '
     'the task is normally addressed in an unsupervised setting where only '
     'defect-free data are available. Normal data themselves are also scarce '
     'in practice: on the Real3D-AD benchmark, each category provides only '
     'four defect-free template scans, all captured by an industrial '
     'binocular blue-light structured-light scanner from real workpieces, '
     'which realistically reflects the cost of acquiring calibrated '
     'high-resolution scans on production lines. Learning-based methods, '
     'including reconstruction-based and feature-embedding approaches, '
     'achieve strong benchmark performance but require hours of per-category '
     'training and must be retrained at every product changeover, which is '
     'impractical in this few-template regime. Registration-based '
     'training-free pipelines instead align a test scan to the normal '
     'templates, compute per-point geometric residuals, and flag regions '
     'whose residuals exceed local normal tolerances; they adapt to new '
     'categories within minutes and remain interpretable throughout. '
     'However, compared with the well-studied registration and feature '
     'representation components, the scoring stage of such pipelines, which '
     'standardizes residuals, fuses per-template evidence, and aggregates '
     'point scores, has received far less attention and still follows a '
     'coarse-grained design with three structural limitations. First, normal '
     'tolerances are pooled over neighborhoods far larger than a small '
     'defect, so a small defect raises the local tolerance at its own '
     'location and largely cancels itself after standardization. Second, '
     'fusion weighted by global registration fitness cannot suppress a '
     'locally misaligned template, because the normalized weights are nearly '
     'uniform in practice and misalignment is a per-point rather than '
     'per-template phenomenon. Third, independent per-point scoring ignores '
     'the spatial contiguity of real defects, inflating false positives '
     'exactly where registration-based methods have historically lagged, '
     'namely point-level metrics. This study aims to upgrade only this '
     'scoring stage at zero training cost while keeping registration and '
     'residual extraction unchanged.'),
    ('Method',
     'This paper proposes geometry-consistent anomaly scoring (GCAS), a '
     'training-free, plug-in scoring module that replaces the conventional '
     'calibration-fusion-aggregation stage of registration-based pipelines. '
     'The pipeline follows the established registration-based skeleton: each '
     'scan is deterministically downsampled to 10 000 points; per-point '
     'normals and curvatures are estimated by local covariance analysis; the '
     'test scan is independently registered to each template by multi-start '
     'RANSAC on fast point feature histogram correspondences followed by '
     'point-to-point iterative closest point refinement; and three per-point '
     'residual channels (nearest-neighbor distance, normal deviation, and '
     'curvature difference) are extracted per template. GCAS then proceeds '
     'in three stages. First, fine-grained tolerance calibration builds a '
     'local null model of defect-free residuals by registering all ordered '
     'template pairs and pooling cross-template residual statistics over '
     'small neighborhoods of K = 10 nearest template points instead of the '
     'conventional 50, so tolerances around fine structures are estimated '
     'accurately and small defects survive standardization. Second, '
     'soft-minimum template fusion applies a point-wise softmax over negated '
     'standardized residuals, namely a Boltzmann-weighted average with '
     'temperature beta that interpolates continuously between the '
     'cross-template mean and the point-wise minimum. This operator '
     'provably satisfies consensus fidelity (responses agreed by all '
     'templates, including true defects visible under every alignment, are '
     'preserved without attenuation), monotone interpolation, and a '
     'quantitative upper bound on single-template contamination, so spurious '
     'residuals from a locally misaligned template are suppressed without '
     'explicitly estimating which template fails where. Third, kNN-graph '
     'spatial consistency regularization performs one step of Gaussian '
     'diffusion on the intrinsic neighbor graph of the test scan with a '
     'dimensionless, scan-adaptive bandwidth tied to the median neighbor '
     'distance, attenuating isolated spikes while consolidating contiguous '
     'defect responses. All three scalars (K, beta, and the smoothing '
     'strength alpha) are frozen once per benchmark by a label-free '
     'normal-only protocol built on leave-one-template-out pseudo scans and '
     'synthesized pseudo defects; no selection step touches test labels, and '
     'no per-category tuning is performed. The module reuses the residual '
     'matrices, nearest-template indices, and neighbor tables already '
     'computed by the pipeline and adds no extra memory.'),
    ('Result',
     'On Real3D-AD (12 categories and 1 206 test scans), under a strict '
     'protocol with three independent random seeds, the official coordinate '
     'grid, and one benchmark-wide parameter setting, GCAS improves the '
     'object-level area under the receiver operating characteristic curve '
     '(AUROC) from 0.893 to 0.928, the point-level AUROC from 0.896 to '
     '0.958, and the point-level average precision (AP) from 0.479 to 0.580 '
     'over the identical pipeline with the conventional scoring stage. '
     'Seed-wise fluctuations are below 0.006 for every metric, an order of '
     'magnitude smaller than the improvements, and two-sided Wilcoxon '
     'signed-rank tests over the twelve paired per-category means reject the '
     'no-difference hypothesis for all four metrics. The object-level result '
     'exceeds the strongest published result (0.859). Point-level AUROC '
     'improves on all twelve categories and point-level AP on ten, with the '
     'largest gains on the categories where the conventional stage is '
     'weakest (airplane: +0.096 object-level AUROC and +0.208 point-level '
     'AP; starfish: +0.094 and +0.256). Stage-wise cumulative ablation '
     'verifies the independent contribution of each stage: soft-minimum '
     'fusion supplies the first object-level gain, fine-grained calibration '
     'adds further object-level improvement, and spatial regularization '
     'contributes most of the point-level gain. The gains also transfer '
     'across benchmarks: on Anomaly-ShapeNet, directly transferring the '
     'frozen setting improves point-level AUROC and AP on 39 of 40 and 35 '
     'of 40 categories respectively, and rerunning the label-free selection '
     'protocol with target templates restores and surpasses object-level '
     'performance (0.926 versus 0.904 for the conventional stage); on '
     'MulSen-AD, point-level gains transfer on 14 of 15 and 13 of 15 '
     'categories. The full scoring stage costs 21.4 ms per sample on a '
     'single CPU core, less than 1% of end-to-end inference time, which '
     'remains dominated by registration.'),
    ('Conclusion',
     'Upgrading only the scoring stage of registration-based 3D anomaly '
     'detection yields significant and reproducible gains at zero training '
     'cost, making such pipelines markedly more practical for few-template '
     'industrial inspection with frequent product changeover. The remaining '
     'failure modes, namely shallow sink defects whose residuals stay within '
     'normal tolerance and geometrically invisible defects, delimit what a '
     'scoring stage alone can achieve and motivate richer geometric and '
     'semantic residual channels, such as frozen features from pretrained '
     'foundation models, as future work.'),
]
...
INTRO = [
    ...
    '如 IMRNet⟦IMRNet⟧复原被掩蔽的正常几何，R3D-AD⟦R3D-AD⟧与 PO3AD⟦PO3AD⟧'
    '对伪异常变体去噪；或通过特征嵌入，将测试特征与预训练特征记忆库匹配，如 M3DM'
    '⟦M3DM⟧、ISMP⟦ISMP⟧与 Group3AD⟦Group3AD⟧。这类方法基准性能优异，但需要针对每个类别进行'
    ...
    '监督；特征嵌入类方法将二维检测中在稠密图像基准 MVTec AD⟦MVTecAD⟧上发展起来的'
    '记忆库范式⟦PatchCore,SPADE⟧移植到点云：M3DM⟦M3DM⟧融合在 ShapeNet⟦ShapeNet⟧上预训练的'
    ...
    'M2P-AD⟦M2PAD⟧、统一多类别建模 MC3D-AD⟦MC3DAD⟧与 SeDiR⟦SeDiR⟧、手工'
    ...
    '要么是全局性的、要么是隐式的。在评分侧，二维检测中 PaDiM⟦PaDiM⟧的逐位置正常'
    '统计建模与 PatchCore⟦PatchCore⟧的分数图高斯平滑均已是成熟做法；而三维点云'
...
S22 = [
    ...
    '@EQ1',
    '@EQ2',
    '其中 σ 以 ρ·|μ| + ε_{0}（ρ = 0.3，ε_{0} = 10^{−6}）为下限，以保证数值稳定。测试残差按',
    '@EQ3',
...
S23 = [
    ...
    '为此，本文对取负的标准化残差做逐点 softmax 融合，即玻尔兹曼加权平均'
    '⟦Boltzmann⟧：',
    '@EQ5',
    '@EQ6',
...
S34 = [
    ...
    # jig-v2：图 2（通栏顶浮）与图 3（栏宽底浮）锚位各推迟一段（fitness
    # 权重验证段后 → 池化规模 K 段后），使锚位随正文落入显示页（v9 原则：
    # 锚位与显示页同页）。
    # jig-v3：公式 OMML 化后正文整体下移约一段，池化规模 K 段后的锚位
    # 滑至第 10 页右栏过低处——图 3 底浮块在本栏剩余空间放不下而跳往
    # 第 11 页，与图 4/图 5 图带同页竞争（LibreOffice 类引擎不执行浮动表
    # 互斥，v10 教训）并在第 10 页右栏留 4.1 cm 悬空。两锚位回移一段
    # （fitness 权重验证段后）作补偿，锚显同页恢复；先文后图不变
    # （图 2 首引在软最小段、图 3 首引即前一段）。
    '@FIG2',
    '@FIG3',
    ...
    # v12：表 6 流位自"为考察各级…"段后提前至此——内嵌表行不会避让底浮
    # 图带（LibreOffice/WPS 类引擎），表 6 需在图 4/图 5 图带矩形之上整体
    # 结束；提前两段后其表尾与图带顶边间隔约两段正文作缓冲，分页微移
    # 亦不致重叠。表 6 的首次正文引用在 3.4 节首段（前页），先文后表不变。
    '@TAB6',
    ...
    '缺陷（补充材料 D）。',
    # v12：图 4/图 5 图带回到 v9 流位（锚位与显示同页时 LibreOffice 类引擎
    # 无"浮动屏障"空洞；v11 试验证明后移至 3.5 节会在锚页右栏与显示页
    # 上方各挖出大洞）。与内嵌表 6 的叠印改由表 6 提前解决（见上）。
    '@FIGBAND45',
...
S35 = [
    ...
    # jig-v2：删除"标出"后的多余空格——该空格恰落在两栏断行与底浮图带交界，
    # 触发 LibreOffice 类引擎丢字（v1 中"值缺"二字被排到游离位置，v2 首轮
    # 渲染中被直接丢弃）；删除后该段断行正常、丢字消失（文字完整性校验工具
    # _qa/text_integrity_check.py 全文通过）。
...
S36 = [
    ...
    # jig-v2：图 6（通栏顶浮）锚位由 2.5 节末移至 2.6 节首段之后。原锚位
    # 落第 12 页右栏而图显示第 13 页页顶，浮动屏障使第 12 页右栏 6 cm 悬空；
    # 图 6 首次引用在 2.5 节（前页），先文后图不变。
    '@FIG6',
    '@TAB7',
    ...
    '上，单一的全基准统一设定并非对象级最优。',
    # jig-v2：图 7（栏宽底浮）锚位由表 7 后移至 ASN 固定设定段之后，随分页
    # 落入其显示页（第 14 页左栏底），表 7 为内嵌三线表可跨栏续排回填。
    '@FIG7',
...
# jig-v3 起公式内容以 OMML 结构化定义（见前文 OMML_EQS，含线性参照注释）。
...
TABLE_META = {
    ...
    'T3': ('表3 逐类点级 AUROC（Real3D-AD）',
           ...
           'Real3D-AD 上未报告点级结果，R3D-AD 与 Template3D-AD 仅报告点级宏均值，'
...
def emit_stream(doc, paras, tables, indent=True):
    for item in paras:
        if item.startswith('@EQ'):
            ...
            # v2(JIG)：按图宽自动选浮排方式——通栏图页顶浮排，栏宽图栏底浮排
            fig = FIGS[int(item[4:])]
...
def _sanitize_headers(doc):
    ...
    根因：JIG 模板 .doc 经 LibreOffice 转 .docx 后，页眉首行段落（“第 卷
    第 期 中国图象图形学报 Vol. ,No.”）的“无边框”直接格式被转成空
    <w:pBdr/>；空 pBdr 不覆盖 Header 样式的 bottom single 边框（逐边继承），
    正稿第 2 页起两行刊头之间因此渲染出一条多余细线（官方模板 .doc 渲染
    仅末行下一条 double 双线，矢量线坐标对照 _qa/render_tpl/jigtpl.pdf）。

    处置：页眉段落中内容为空的 pBdr 全边显式置 none；带内容的 pBdr（末行
    bottom double 双线）保持不动。渲染结果与官方模板逐线一致。"""
    for part in doc.part.package.iter_parts():
        if not re.search(r'header\d+\.xml$', str(part.partname)):
            continue
        for pPr in part.element.iter(qn('w:pPr')):
            pbdr = pPr.find(qn('w:pBdr'))
            if pbdr is None or len(pbdr):
                continue
...
def build():
    ...
    init_cite_labels(entries)      # 著者-出版年标注表（含 a/b 消歧）
    tables = load_md_tables(DRAFT)

    doc = Document(TEMPLATE)

    # —— jig-v3：页眉残线净化（见函数 docstring）——
    _sanitize_headers(doc)

    # —— 合成 JIG 分节符（模板段落级分节均为单栏，双栏节需程序构造）——
    # 首节：模板 sectPr0（21×28.8 cm、边距 2.54/2.0、titlePg、页眉页脚引用、
    # docGrid 312）原样克隆；双栏/单栏正文节在其基础上改列数并去首页引用。
    para_sects = [s for p in doc.element.body.iterchildren(qn('w:p'))
                  for s in p.findall(qn('w:pPr') + '/' + qn('w:sectPr'))]
    assert para_sects, 'JIG 模板未含段落级分节符'
    sect_front = copy.deepcopy(para_sects[0])

    def _derive_sect(ncols):
        s = copy.deepcopy(sect_front)
        t = s.find(qn('w:type'))
        if t is None:
            t = OxmlElement('w:type')
            s.insert(0, t)
        ...
        if cols is None:
            ...
            # cols 需位于 pgMar 之后、docGrid 之前（OOXML 顺序），插在
            # pgMar 后即可
            pgmar = s.find(qn('w:pgMar'))
            pgmar.addnext(cols)
        cols.set(qn('w:num'), str(ncols))
        cols.set(qn('w:space'), '424')
        if ncols == 1:
            cols.attrib.pop(qn('w:num'), None)
            cols.attrib.pop(qn('w:sep'), None)
        return s

    sect_2col = _derive_sect(2)
    sect_1col = _derive_sect(1)

    # —— 清空正文，并把 body 末级 sectPr 统一为 JIG 单栏几何 ——
    ...
    body.append(copy.deepcopy(sect_1col))

    # ============ 第 1 节：单栏头部（JIG 模板体例） ============
    # 顶行：中图法分类号 / 文献标识码 / 文章编号（黑体小五，标签加粗）
    p = doc.add_paragraph()
    pset(p, snap=False, line=240, after=3)
    r = p.add_run('中图法分类号：')
    set_font(r, HEI, HEI, 9, bold=True)
    r = p.add_run('TP391.4【建议值，待作者确认】　　')
    set_font(r, TNR, HEI, 9)
    r = p.add_run('文献标识码：')
    set_font(r, HEI, HEI, 9, bold=True)
    r = p.add_run('A　　')
    set_font(r, TNR, HEI, 9)
    r = p.add_run('文章编号：')
    set_font(r, HEI, HEI, 9, bold=True)
    r = p.add_run('1006-8961(2026)-【出版时由编辑部填写】')
    set_font(r, TNR, HEI, 9)

    # 论文引用格式行（模板要求）
    p = doc.add_paragraph()
    pset(p, snap=False, line=240, after=6)
    r = p.add_run('论文引用格式：')
    set_font(r, HEI, HEI, 9, bold=True)
    r = p.add_run('【作者一姓名】，【作者二姓名】，【作者三姓名】. 2026. '
                  + ZH_TITLE + '. 中国图象图形学报，【卷(期)：起止页码，'
                  '录用后由编辑部补】[DOI: 10.11834/jig.【编号】]')
    set_font(r, TNR, SONG, 9)

    p = doc.add_paragraph()
    pset(p, align=WD_ALIGN_PARAGRAPH.CENTER, snap=False, before=12, after=6,
         line=300)
    emit_runs(p, ZH_TITLE, size=22, ea=HEI)

    p = doc.add_paragraph()
    pset(p, align=WD_ALIGN_PARAGRAPH.CENTER, snap=False, before=6, after=3,
         line=280)
    emit_runs(p, '【作者一姓名】^{1*}，【作者二姓名】^{1,2}，【作者三姓名】^{2}',
              size=14, ea=SONG)

    p = doc.add_paragraph()
    pset(p, align=WD_ALIGN_PARAGRAPH.CENTER, snap=False, after=8, line=240)
    emit_runs(p, '1.【第一单位全称（学院/研究所），省份 城市 邮编】；'
              '2.【第二单位全称，省份 城市 邮编】', size=7.5, ea=SONG)

    # 摘要：目的/方法/结果/结论四要素，引导词加粗（模板体例）
    p = doc.add_paragraph()
    pset(p, snap=False, before=4, line=240)
    r = p.add_run('摘　要：')
    set_font(r, HEI, HEI, 9, bold=True)
    for i, (label, text) in enumerate(ABSTRACT_ZH_PARTS):
        r = p.add_run(('' if i == 0 else '') + label + '　')
        set_font(r, HEI, HEI, 9, bold=True)
        r = p.add_run(text)
        set_font(r, TNR, SONG, 9)
    ...
    pset(p, align=WD_ALIGN_PARAGRAPH.CENTER, snap=False, before=10, after=6,
         line=300)
    emit_runs(p, '**' + EN_TITLE + '**', size=14, ea=SONG)

    p = doc.add_paragraph()
    pset(p, align=WD_ALIGN_PARAGRAPH.CENTER, snap=False, after=3, line=260)
    emit_runs(p, '【Surname Given-name】^{1*}, 【Surname Given-name】^{1,2}, '
              '【Surname Given-name】^{2}', size=14, ea=SONG)

    p = doc.add_paragraph()
    pset(p, align=WD_ALIGN_PARAGRAPH.CENTER, snap=False, after=6, line=240)
    emit_runs(p, '*1.【Department, University】, 【City】【Postcode】, China; '
              '2.【Department, University】, 【City】【Postcode】, China*',
              size=7.5, ea=SONG)

    # Abstract：Objective/Method/Result/Conclusion 四要素，约 1000 词
    p = doc.add_paragraph()
    pset(p, snap=False, line=240)
    r = p.add_run('Abstract: ')
    set_font(r, TNR, HEI, 9, bold=True)
    for i, (label, text) in enumerate(ABSTRACT_EN_PARTS):
        r = p.add_run(label + '  ')
        set_font(r, TNR, HEI, 9, bold=True)
        r = p.add_run(text + ('  ' if i < len(ABSTRACT_EN_PARTS) - 1 else ''))
        set_font(r, TNR, SONG, 9)
    ...
    # jig-v2：图 1 流位由 1.1 节末推迟至 1.2 节第 2 段之后。JIG 头区
    # （英文长摘要）比 OEP 版多约一页，图 1 显示页后移至第 5 页页顶，
    # 而锚位仍停留在第 4 页时 LibreOffice 类引擎把锚段连同其后正文一并
    # 推往显示页（浮动屏障），第 4 页右栏 5.4 cm、第 5 页左栏 15.8 cm
    # 悬空。锚位推迟两段后自然落在显示页内，正文连续回填（v9 原则）。
    emit_stream(doc, S22[:2], tables)
    ...
    heading1(doc, '3　结　论')
    emit_stream(doc, S4, tables)

    # 数据与程序可用性说明并入结论末尾（JIG 无固定栏目；模板"特别提醒"鼓励
    # 提供可验证数据与代码附件）
    p = doc.add_paragraph()
    pset(p, fli_chars=2, snap=False, line=240)
    emit_runs(p, DATA_AVAIL, size=10.5)

    # 致谢（可选栏目，模板体例：楷体小五；无则删）
    p = doc.add_paragraph()
    pset(p, before=6, after=2, snap=False, line=240)
    r = p.add_run('致　谢')
    set_font(r, HEI, HEI, 9)
    r = p.add_run('　【如无需致谢请删除本段：对提出重要指导意见、提供研究数据/'
                  '仪器设备/实验条件的机构和人员表达感谢。】')
    set_font(r, TNR, KAI, 9)

    # 参考文献(References)：按第一作者字母顺序排序（模板要求）
    p = doc.add_paragraph()
    pset(p, before=8, after=4, snap=False, line=260)
    emit_runs(p, '参考文献(References)', size=10.5, ea=HEI, ascii_=HEI)
    missing_report = []
    if len(CITE_KEYS) != len(entries):
        unused = [k for k in entries if k not in CITE_KEYS]
        print(f'[warn] bib 中未被正文引用的条目 {len(unused)} 个：{unused}')
    rendered = {}
    for key in CITE_KEYS:
        etype, fields = entries[key]
        rendered[key] = jig_format(key, etype, fields)
    for key in sorted(rendered, key=lambda k: rendered[k][0].lower()):
        text, missing = rendered[key]
        if missing:
            lab = CITE_LABELS[key]
            missing_report.append((f'{lab[0]}，{lab[1]}', key, missing))
        p = doc.add_paragraph()
        pset(p, snap=False, line=200, ind_left=284, hanging=284)
        r = p.add_run(text)
    ...
    # ============ 末节：单栏（作者简介 + 排版占位说明） ============
    p = doc.add_paragraph()
    pset(p, before=8, after=3, snap=False, line=260)
    emit_runs(p, '**作者简介**', size=10.5)
    for bio in [
        '第一作者：【姓名】，【出生年】年生，【性别】，【职称】，研究方向为'
        '【研究方向】。E-mail: 【email@example.edu.cn】',
        '通信作者：【姓名】，通信作者，【性别】，【职称】，主要研究方向为'
        '【研究方向】。E-mail: 【email@example.edu.cn】',
        '其他作者：【姓名】，【性别】，【职称】，主要研究方向为【研究方向】。'
        'E-mail: 【email@example.edu.cn】（按作者顺序列出）',
    ]:
        p = doc.add_paragraph()
        pset(p, snap=False, line=240, after=3)
        emit_runs(p, bio, size=9)

    p = doc.add_paragraph()
    pset(p, before=10, after=2, snap=False, line=260)
    emit_runs(p, '**——以下为排版占位说明（投稿前请删除本块）——**', size=10.5)
    for note in [
        '1. 题名方案：现用「' + ZH_TITLE + '」（18 字，无缩写词，符合 JIG 题名'
        '建议）。备选：「几何一致评分驱动的免训练点云异常检测」。可再议。',
        '2. 全部【】括注均为待作者补齐的占位符：作者姓名/单位/邮箱、基金项目、'
        '作者简介、中图法分类号确认、论文引用格式行的作者名。',
        '3. 首页页脚的收稿日期/修回日期/基金项目（中英文）与页眉卷期由编辑部/'
        '投稿系统填写，模板占位已保留；基金项目中英文名称可在学报网站下载中心'
        '核对。',
        '4. 公式 (1)–(11) 已按 Word 原生公式对象（OMML，"插入→公式"工具）'
        '排版（一式一号，变量斜体、函数名直立由公式引擎自动式样）；如编辑部'
        '要求 MathType 格式，可在 Word 中逐式转换（内容无需重录）。正文行内'
        '符号为带格式化上下标的文本。',
        '5. 附录 A–I 及附图三张拟作为附件材料随投稿系统提交（JIG"特别提醒"'
        '鼓励上传可验证的实验数据与程序代码附件）；正文仅保留结论性引用，'
        '相应"补充材料"字样如编辑部要求可改为"附件"。',
        '6. 参考文献按第一作者字母序排列、著者-出版年制；会议条目的会议录'
        '全称与出版地/出版者按通行著录填制，个别小型会议以 [S.l.]: [s.n.] '
        '占位；英文题名已统一为句首大写式（与 JIG 2021 著录样例一致，缩写/'
        '专名保留原大小写）；全部条目 DOI 待作者按刊方要求补充（本刊格式'
        '支持 [DOI:] 项）。',
        '7. 正文引用采用（作者，出版年）形式；同一作者同年多篇以 a/b 区分'
        '（本稿两组：Liang 等，2025a=ISMP/2025b=DUS-Net；Qi 等，2017a='
        'PointNet/2017b=PointNet++），与文献表一一对应。',
        '8. 双盲评审提示：JIG 网站如要求匿名稿，请另存一份删除作者/单位/'
        '基金/作者简介信息的版本。',
    ]:
        p = doc.add_paragraph()
    ...
    # ============ 页脚占位替换（JIG 首页脚注：收稿/修回日期 + 基金项目） ============
    sec0 = doc.sections[0]
    try:
        for fp in sec0.first_page_footer.paragraphs:
            txt = fp.text
            if '基金项目' in txt and '规范中文全称' in txt:
                for r in list(fp.runs):
                    if '基金项目的规范中文全称' in r.text:
                        r.text = '【基金项目规范中文全称（项目编号：待作者补充；'
                        r.text += '不同基金之间用分号隔开）】'
            if 'Supported by' in txt:
                for r in list(fp.runs):
                    if '基金项目的英文全称' in r.text:
                        r.text = '【Supported by 基金项目英文全称（可在学报网站'
                        r.text += '下载中心核对，待作者补充）】'
    except Exception as e:      # 页脚结构与预期不符时不阻塞主流程
        print('[warn] 页脚占位替换跳过：', e)
    ...
    n_om_par = 0
    n_om = 0
    for cp in chk.paragraphs:
        k = len(cp._p.findall('.//' + qn('m:oMath')))
        if k:
            n_om_par += 1
            n_om += k
    print(f'[ok] 已生成 {OUT}')
    print(f'     段落 {n_par} | 顶级表格 {n_tbl}（内嵌三线表 7 + 单图浮动容器 6 '
          f'+ 双图图带 1）| 图片 {n_img} | 分节 {n_sect} | '
          f'编号公式 {EQN_COUNT[0]}（OMML 段 {n_om_par}/数学区 {n_om}）| '
          f'参考文献 {len(CITE_KEYS)}')
    assert n_img == 8, f'图片数应为 8，实际 {n_img}'
    assert n_tbl == 14, \
        f'顶级表格数应为 14（内嵌三线表 7 + 单图容器 6 + 图带 1，v11），' \
        f'实际 {n_tbl}'
    ...
    assert len(CITE_KEYS) == 55, f'引用数应为 55，实际 {len(CITE_KEYS)}'
    assert EQN_COUNT[0] == 11, f'编号公式应为 11（一式一号），实际 {EQN_COUNT[0]}'
    assert n_om_par == 11 and n_om == 13, \
        f'OMML 公式段应 11、数学区应 13（式 7/8 各含条件+结论两区），' \
        f'实际 {n_om_par}/{n_om}'
    # 著者-年制一致性终检：正文标注（含 a/b 后缀）与文献表一一对应
    dup = len(CITE_LABELS) - len({CITE_LABELS[k] for k in CITE_LABELS})
    assert dup == 0, f'存在 {dup} 组未消解的重名标注'
    if missing_report:
        print('     [文献待补字段]')
        for lab, key, missing in missing_report:
            print(f'       ({lab}) {key}: {"、".join(sorted(set(missing)))}')
