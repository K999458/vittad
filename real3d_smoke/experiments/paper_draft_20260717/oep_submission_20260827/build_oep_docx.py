...
# -*- coding: utf-8 -*-
"""
build_oep_docx.py —— 《光学 精密工程》(OEP) 投稿初稿构建脚本
...
输出（均写入本目录，不改动任何输入）：
  ...
  名词短语式双语题注（图1/Fig.1、表1/Tab.1），分图与图例说明补齐中英对照两行；
  ...
  数据零变化。v1–v11 保留）
  （v13：公式 OMML 化 + 文字勘误——作者审阅 v12 指出四类问题。①公式
  表示：式 (1)–(11) 原为 Unicode 文本占位（上下标靠字符格式、分式写成
  斜线、Σ 上下限挤在行内，式 (3)/(9)/(10) 线性写法过长有栏内折行风险），
  全部改为 Word 原生公式对象（OMML，即 Word"插入→公式"工具的格式）
  重排——真分式、求和号上下限置于其上下（undOvr）、结构化上下标与
  波浪饰符，函数名 exp/min/mean/std/median 直立、变量斜体（公式引擎
  自动式样）；公式居中制表位由 3.9 cm 改为 4.17 cm（栏宽 8.34 cm 的
  真中点），编号仍右贴栏缘；式 (9) 集合差由字面反斜杠改为 ∖（U+2216），
  式 (10) 高斯核指数改为真分式，式 (7)/(8) 的"时，"以正文宋体夹排于
  条件与结论两段数学区之间；句末标点（，/。）移出公式对象、以正文
  字体紧随其后。②英文大小写：55 条参考文献英文题名由原文 Title Case
  统一转 GB/T 7714 句首大写式（sentence case；缩写/驼峰/含数字词与
  Eyecandies 等专名保留原大小写，冒号/破折号后小写），正文孤例
  "k-NN 索引"统一为全文通用写法"kNN"。③标点："命题 1."的半角句点
  紧贴中文，改为"命题 1"后接全角空格（定理体例）。④多余空格：3.5 节
  "标出 真值缺陷区域"中文间的多余半角空格删除。正文其余文字与图表
  数据零变化。v1–v12 保留）
...
TEMPLATE = os.path.join(BASE, 'downloads', '中文论文模板 (2024).docx')
DRAFT = os.path.join(ROOT, 'DRAFT_v1_zh.md')
BIB = os.path.join(ROOT, 'references.bib')
FIGDIR = os.path.join(ROOT, 'figures')
OUT = os.path.join(BASE, 'GCAS_光学精密工程_排版初稿_v13_20260831.docx')

# 页面几何（由模板 sectPr 实测）：A4，正文宽 9694 twip = 17.13 cm；
# 双栏栏宽 (9694-236)/2 = 4729 twip = 8.34 cm
FULL_W = Cm(16.8)      # 通栏图/表宽度（留少许余量）
COL_W = Cm(8.2)        # 栏宽图宽度
COL_W_TW = 4729        # 栏宽（twip），用于栏内表格
FULL_W_TW = 9694       # 全宽（twip）
...
# 参考文献英文题名统一为 GB/T 7714 句首大写式（sentence case，v13）。
# 启发式保护：含数字或 ≥2 个大写字母的词段（缩写、驼峰、型号，如 3D、
# Real3D-AD、PointNet++、MVTec、FPFH、DRÆM）原样保留；连字符复合词任一
# 段受保护则整词保留（Point-BERT、3-D）；无法启发式识别的单大写专名
# 列入 TITLE_PROTECT 兜底。冒号/破折号后的子题名首词一律小写（GB/T
# 7714 示例体例，如"HCI beyond the GUI: design for haptic…"）。
...
def heading1(doc, text):
    ...
    emit_runs(p, text, size=14, ea=SONG, ascii_=TNR)
    return p
...
# ------------------------------------------- OMML 公式（Word 原生公式对象）
# v13：式 (1)–(11) 由 Unicode 文本占位改为 Word"插入→公式"工具的原生
# OMML 对象：真分式、求和号上下限（undOvr）、结构化上下标与波浪饰符；
# 变量由公式引擎按规则自动斜体，函数名（exp/min/mean/std/median）与
# 文字性记号（top-k、obj）直立。句末标点（，/。）与式 (7)/(8) 的
...
def mo(t):
    """直立体数学 run（函数名）。"""
    return (f'<m:r><m:rPr><m:sty m:val="p"/></m:rPr>{_M_RPR}'
...
def equation(doc, num):
    """编号公式段：居中制表位（4.17 cm = 栏宽 8.34 cm 中点）排 OMML
    数学区与夹排文字，编号以右对齐制表位贴双栏栏缘。"""
    EQN_COUNT[0] += 1
    p = doc.add_paragraph()
    pset(p, snap=False, before=3, after=3, line=260)
    ts = p.paragraph_format.tab_stops
    ts.add_tab_stop(Cm(4.17), WD_TAB_ALIGNMENT.CENTER)
    ts.add_tab_stop(Cm(8.34), WD_TAB_ALIGNMENT.RIGHT)   # 编号贴双栏栏缘
    r = p.add_run('\t')
...
def caption_pair(doc, zh, en, size=9):
    ...
    pset(p2, align=WD_ALIGN_PARAGRAPH.CENTER, snap=False, before=0, after=3,
         line=240)
    keep_next(p2)
    emit_runs(p2, en, size=size, ea=SONG)
    return p1, p2


def table_note(doc, text, size=7.5):
    p = doc.add_paragraph()
...
def add_figure(doc, png, width, zh_cap, zh_sub, en_cap, en_sub):
    ...
    for i, text in enumerate(lines):
        ...
        emit_runs(p, text, size=9, ea=SONG)
...
def add_floating_figure(doc, png, width, zh_cap, zh_sub, en_cap, en_sub,
    ...
    box_tw = '4729' if column else '9694'   # 栏宽 8.34 cm / 正文全宽 17.13 cm
...
ABSTRACT_EN = (
    ...
    'localize defects using defect-free data only, and '
    'registration-based pipelines are attractive for this setting since they '
    'require no training and adapt quickly to new categories. However, the '
    'scoring stage that converts per-template residuals into anomaly scores '
    'has received little attention: normal tolerances are pooled over '
    'neighborhoods far larger than a small defect, fusion weighted by global '
    'registration fitness cannot suppress a locally misaligned template, and '
    'independent point scoring ignores the spatial contiguity of real '
    'defects. To address these issues, this paper proposes '
    'Geometry-Consistent Anomaly Scoring '
    '(GCAS), a training-free scoring module that replaces the conventional '
    ...
    'calibration stage estimates local normal tolerances over small '
    'neighborhoods (K = 10) to preserve small defects. Second, a soft-minimum '
    'template fusion stage applies a point-wise softmax over negated '
    'standardized residuals, suppressing spurious residuals from misaligned '
    'templates while preserving cross-template consensus. Third, a kNN-graph '
    'spatial consistency regularization stage consolidates contiguous defect '
    'responses over the intrinsic neighbor graph of the test scan with a '
    'dimensionless Gaussian kernel. Experimental results show that, under a '
    'strict protocol on Real3D-AD (three seeds, official '
    'coordinate grid, one benchmark-wide setting), GCAS improves the '
    'object-level area under the receiver operating characteristic curve '
    '(AUROC) from 0.893 to 0.928, the point-level AUROC from 0.896 to 0.958, '
    'and the point-level average precision (AP) from 0.479 to 0.580 over the '
    'identical pipeline with the '
    'conventional scoring stage, exceeding the strongest published '
    'object-level result (0.859); the point-level gains transfer to '
    'Anomaly-ShapeNet and MulSen-AD, and the scoring stage adds less than 1% '
    'of end-to-end inference time. These results indicate that upgrading only '
    'the scoring stage yields significant '
    'gains at zero training cost, making registration-based 3D anomaly '
    'detection more practical for few-template industrial inspection with '
...
S21 = [
    ...
    '⟦Hoppe,Pauly⟧，并保留测试扫描的 kNN 索引与距离表供后续阶段复用。测试扫描独立'
...
S22 = [
    ...
    '退化；K 过大则过度平滑（第 3.4 节）。K 在全基准上一次固定，从不按类别调参'
    '（第 2.5 节）。',
]
...
S23 = [
    ...
    '实践中近乎均匀（第 3.4 节），无法有效抑制配准不良的模板；其二，更根本的是，配准'
    ...
    '均值不存在这样的界：在实践中观测到的近乎均匀权重下（见第 3.4 节），单行抬高 Δ 会'
    '使融合分数增加约 Δ/M，随 Δ 无界增长。',

    '反观各替代方案：硬最小值（β → ∞）对离群值的抑制最强，但丢弃了跨模板一致性，'
    '对点级指标造成了明显的性能损失（第 3.4 节）；截断或阈值化的 fitness 加权仍受限于'
...
S25 = [
    ...
    '；协议在三个种子上各自的选择、以及所选设定在正式协议下的表现见第 3.4 节；'
    '第 3.6 节将同一程序应用于另外两个基准。',
]
...
S26 = [
    ...
    '之外不新增内存占用，整体开销仍由配准主导；实测分解见第 3.5 节。',
]

S31 = [
    'Real3D-AD⟦Reg3D-AD⟧包含 12 个类别，每类提供 4 个无缺陷模板，'
    '共有 1 206 个测试扫描点云；如第 1 节所述，全部点云均为高精度工业扫描仪对'
    ...
    '见补充材料 E）。两处诊断性分析使用 PCD 原生 10K 点网格并已在文中注明（第 3.4、'
    '3.5 节）；两种网格的数值均包含在公开的逐样本输出中。',

    '在评测协议上，本文报告的所有数值均为整条管线（含配准）在种子 '
    '{42, 104771, 209500} 下'
    '三次独立重复实验的均值；逐种子数值与标准差见补充材料。所有随机流均由运行种子派生'
    '。超参数 (K, β, α) 全基准共用一套设定、从不按类别调参；第 2.5 节 normal-only '
    '协议的实际选择及其对结果的影响见第 3.4 节。',
    ...
    'fitness 平方加权则对应第 2.3 节所分析的默认配准置信度加权方案；点级指标在补充'
    ...
    '双精度浮点计算。全部计时均在单个 CPU 工作进程上实测，报告于第 3.5 节。',
]
...
S33 = [
    ...
    '破折号（—）表示该指标未见报告。本文两种方法（常规管线与 GCAS）沿用第 3.2 节的'
...
S34 = [
    ...
    '收益甚微。丢弃跨模板一致性的算子在点级性能上付出代价，这与第 2.3 节的理论分析相'
    ...
    '（表 6）。报告设定取 β = 1；第 2.5 节已执行的 normal-only 协议在三个种子中的两个'
    ...
    '此外，在三个随机种子上分别运行第 2.5 节的 normal-only 协议以检验参数选择：两个'
    ...
    '配准失败与真实缺陷，因为第 2.3 节的共识机制依赖于多模板的多数表决效应。',
]
...
S36 = [
    ...
    '基准提供的正常模板，重复第 2.5 节的无标签选参流程（表 7）。',
    '@TAB7',
    # v9：图 7 的流位后移至表 7 之后——表 7 为内嵌三线表、可按行跨栏续排，
    # 恰好回填图 7 让出的栏位；图 7 浮动显示位置不变（次页栏底、紧邻引用）
    '@FIG7',
    ...
    '1.00 → 0.51、headset1 0.93 → 0.25）。这两种效应均符合第 2.2 节与第 3.4 节的机制'
...
S4 = [
    ...
    '（第 3.6 节）。这表明仅通过优化评分阶段，即可在零训练成本下获得显著的性能提升。',

    '在局限方面，本文默认参数由诊断性扫描一次性冻结、全基准统一使用，虽具通用性，'
    '但尚未做到完全无标签自适应；此外，GCAS 的整体处理速度受限于上游配准环节，'
    '单样本耗时约 16 s。评分阶段仅能重组与挖掘残差通道所提供的信息，这正是第 3.5 节'
    '浅凹陷失败案例'
    '与第 3.6 节几何不可见缺陷共同划定的性能边界。在后续工作中引入更为丰富的几何与'
...
# v13 起公式内容以 OMML 结构化定义（见前文 OMML_EQS，含线性参照注释）。
...
TABLE_META = {
    'T1': ('表1 12 类宏均值（三种子均值±标准差，官方坐标网格）',
           'Tab.1 Macro average over 12 categories (mean±std over three '
    ...
    'T2': ('表2 逐类对象级 AUROC（Real3D-AD）',
           'Tab.2 Object-level AUROC by category (Real3D-AD)',
    ...
    'T3': ('表3 逐类点级 AUROC（Real3D-AD）',
           'Tab.3 Point-level AUROC by category (Real3D-AD)',
    ...
    'T4': ('表4 12 类宏汇总',
           'Tab.4 Macro summary over 12 categories',
    ...
    'T5': ('表5 融合算子比较（三种子均值±标准差，官方坐标网格）',
           'Tab.5 Fusion operator comparison (three-seed mean±std, official '
           'coordinate grid)',
           'K = 50 校准、无空间正则化。', COL_W_TW, 6.5, 0.30),
    'T6': ('表6 主路径逐级累积消融（三种子均值±标准差，官方坐标网格）',
           'Tab.6 Per-stage cumulative ablation along the main path '
           '(three-seed mean±std, official coordinate grid)',
           '第 1 行是表 1 的常规阶段，第 4 行是完整 GCAS。', COL_W_TW, 6.5, 0.34),
    'T7': ('表7 官方坐标网格下的迁移表现',
           'Tab.7 Transfer performance on the official coordinate grids',
           '“固定”指把冻结的 Real3D-AD 设定 (10, 1, 1) 直接沿用；“重选”指只用目标基准'
           '的模板运行第 2.5 节的无标签协议，不触及任何测试数据。Anomaly-ShapeNet 为'
...
def build():
    ...
    # —— 收集模板分节符克隆 ——
    para_sects = doc.element.body.findall(
        qn('w:p') + '/' + qn('w:pPr') + '/' + qn('w:sectPr'))
    ...
    emit_runs(p, '文章编号：【出版时由编辑部填写】    doi：【编辑部填写】', size=7.5,
              ea=SONG, ascii_=TNR)
    ...
    emit_runs(p, '（1.【第一单位（学院/研究所），省份 城市 邮编】；'
              '2.【第二单位，省份 城市 邮编】）', size=10.5, ea=SONG)

    p = doc.add_paragraph()
    pset(p, snap=False, before=4, line=240)
    emit_runs(p, '摘　要：', size=9, ea=HEI, ascii_=HEI)
    r = p.add_run(ABSTRACT_ZH)
    set_font(r, TNR, SONG, 9)

    p = doc.add_paragraph()
    pset(p, snap=False, line=240)
    emit_runs(p, '关　键　词：', size=9, ea=HEI, ascii_=HEI)
    r = p.add_run(KEYWORDS_ZH)
    set_font(r, TNR, SONG, 9)

    p = doc.add_paragraph()
    pset(p, snap=False, line=240, after=8)
    ...
    pset(p, align=WD_ALIGN_PARAGRAPH.CENTER, snap=False, before=10, after=6,
         line=300)
    emit_runs(p, EN_TITLE, size=16, ea=SONG)
    ...
    emit_runs(p, '【SURNAME Given-name】^{1}*, 【SURNAME Given-name】^{1,2}, '
              '【SURNAME Given-name】^{2}', size=12, ea=SONG)

    p = doc.add_paragraph()
    pset(p, align=WD_ALIGN_PARAGRAPH.CENTER, snap=False, after=2, line=240)
    ...
    emit_runs(p, '*Corresponding author, E-mail: 【email@example.edu.cn】',
              size=10.5, ea=SONG)
    ...
    heading1(doc, '2 几何一致异常评分（GCAS）')
    heading2(doc, '2.1 问题设定与管线总览')
    emit_stream(doc, S21, tables)
    # v7：图 1 页顶通栏浮排；v9 起其元素推迟至 2.1 节末，使流位置贴近
    # 实际显示页，前页栏底不再留"浮动屏障"空白（见文件头 v9 注记）
    add_floating_figure(doc, *FIGS[1])

    heading2(doc, '2.2 第 1 级：细粒度公差校准')
    emit_stream(doc, S22, tables)
    heading2(doc, '2.3 第 2 级：软最小模板融合')
    emit_stream(doc, S23, tables)
    heading2(doc, '2.4 第 3 级：kNN 图空间一致性正则化')
    emit_stream(doc, S24, tables)
    heading2(doc, '2.5 参数选择协议')
    emit_stream(doc, S25, tables)
    heading2(doc, '2.6 评分流程、输出、复杂度与成本')
    emit_stream(doc, S26, tables)

    heading1(doc, '3 实验与结果分析')
    heading2(doc, '3.1 实验设置')
    emit_stream(doc, S31, tables)
    heading2(doc, '3.2 主结果')
    for item in S32:
        if item == '@TAB1':
            emit_table(doc, 'T1', tables)
        else:
            body_para(doc, item)
    heading2(doc, '3.3 与已发表方法的对比')
    for item in S33:
        if item == '@TABLES234':
    ...
    for item in S35:
        if item == '@FIG6':
            ...
            body_para(doc, item)
    heading2(doc, '3.6 跨基准迁移')
    emit_stream(doc, S36, tables)

    heading1(doc, '4 结论')
    emit_stream(doc, S4, tables)

    # 数据与程序可用性（意见 52：替代正文"将开源"期货式表述）
    p = doc.add_paragraph()
    ...
    p = doc.add_paragraph()
    pset(p, before=8, after=2, snap=False, line=260)
    emit_runs(p, '**作者贡献声明：**', size=10.5)
    for line in CONTRIB:
        p = doc.add_paragraph()
        pset(p, snap=False, line=240)
        emit_runs(p, line, size=9)
    ...
    emit_runs(p, '参考文献：', size=10.5, ea=HEI, ascii_=HEI)
    missing_report = []
    ...
    # ============ 末节：单栏（作者简介 + 创新点 + 排版占位说明） ============
    p = doc.add_paragraph()
    pset(p, before=8, after=3, snap=False, line=260)
    emit_runs(p, '**作者简介：**', size=10.5)
    for bio in [
        '【作者一姓名】（【出生年】-），【性别】，【籍贯】人，【学历】，【职称】，'
        '【入学年份/获学位年份及学校】，主要从事【研究方向】方面的研究。'
        'E-mail: 【email@example.edu.cn】',
        '【通讯作者姓名】（【出生年】-），【性别】，【籍贯】人，【学历】，【职称】，'
        '【博士生导师等】，主要从事【研究方向】方面的研究。'
        'E-mail: 【email@example.edu.cn】',
    ]:
        p = doc.add_paragraph()
    ...
    p = doc.add_paragraph()
    pset(p, before=6, after=2, snap=False, line=260)
    emit_runs(p, '**文章创新点介绍（约 300 字）：**', size=10.5)
    p = doc.add_paragraph()
    pset(p, snap=False, line=240, fli_chars=2)
    emit_runs(p, INNOVATION, size=9)

    p = doc.add_paragraph()
    pset(p, before=6, after=2, snap=False, line=260)
    emit_runs(p, '**通信信息（供编辑部联系，发表时不刊出）：**', size=10.5)
    p = doc.add_paragraph()
    pset(p, snap=False, line=240)
    emit_runs(p, '通信地址：【省 市 区 街道门牌 单位 部门，邮编】；联系固话：'
              '【0XXX-XXXXXXXX】；手机：【1XXXXXXXXXX】（第一作者或通讯作者）。',
              size=9)
    ...
    for note in [
        '1. 题名方案：现用「' + ZH_TITLE + '」（18 字）。备选：「基于几何一致评分的'
        '免训练三维点云异常检测」（20 字，含 OEP 题名建议避免的“基于”一词）、'
        '「几何一致评分驱动的免训练点云异常检测」。可再议。',
        '2. 全部【】括注均为待作者补齐的占位符：作者姓名/单位/邮箱、基金项目（本刊'
        '要求必须有基金支持）、作者简介（需附照片）、通信地址与电话、中图分类号确认。',
        '3. 首页页脚的收稿日期/修订日期/基金项目、页眉的卷期年月由编辑部/投稿系统'
        '填写；奇数页页眉的作者与题名已按本稿占位。',
        '4. 公式 (1)–(11) 已按 Word 原生公式对象（OMML，“插入→公式”工具）排版：'
        '真分式、求和号上下限、结构化上下标与波浪饰符，公式居中、编号右贴栏缘；'
        '如编辑部要求 MathType 格式，可用 MathType 直接转换 Word 原生公式。'
        '正文行内符号仍为 Unicode 文本＋上下标字符格式。',
        '5. 附录 A–I 及附图三张拟作为补充材料提交（见正文“补充材料说明”）。',
    ...
    sec0 = doc.sections[0]
    try:
        for hp in sec0.header.paragraphs:          # 奇数页页眉（default）
            for r in hp.runs:
                if '张舞杰' in r.text:
                    r.text = r.text.replace('张舞杰', '【第一作者】')
                if '自动影像测量系统的设计' in r.text:
                    r.text = r.text.replace('自动影像测量系统的设计', SHORT_TITLE)
        for fp in sec0.first_page_footer.paragraphs:   # 首页脚注区
            if '基金项目' in fp.text:
                keep_label = None
                for r in list(fp.runs):
                    if '基金项目' in r.text and keep_label is None:
                        keep_label = r
                    elif keep_label is not None:
                        r.text = ''
                if keep_label is not None:
                    keep_label.text = '基金项目：'
                    r = fp.add_run('【基金项目名称及编号（本刊只刊登有基金支持的'
                                   '项目，待作者补充）】')
                    set_font(r, TNR, SONG, 7.5)
    except Exception as e:      # 页眉页脚结构与预期不符时不阻塞主流程
        print('[warn] 页眉/页脚占位替换跳过：', e)
    ...
    assert n_sect == 5, f'分节数应为 5（v7 仅余头部/表区/末节分节），实际 {n_sect}'
